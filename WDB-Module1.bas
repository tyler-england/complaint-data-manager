...
For i = 75 To 4 Step -1 'find last complaint cat row (not counting "Complaint Categories"
    If wSheet.Cells(i, catCol).Value <> 0 And wSheet.Cells(i, catCol).Value <> Sheet4.Range("M1").Value Then
        Exit For
    End If
Next i
...
If errorMsg = False Then
    ...
    For Each cell In chkRng 'cells in supplier column
        If cell.Value <> Sheet4.Range("Q1").Value Then
            errorMsg = True
            Exit For
        End If
    Next
...
If errorMsg = False And rcCol > 0 Then
    ...
    For Each cell In chkRng 'cells in supplier column
        If cell.Value <> Sheet4.Range("O1").Value Then
            errorMsg = True
            Exit For
        End If
    Next
